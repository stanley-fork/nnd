...
bitflags! { pub struct ArrayFlags: u8 {
    ...
    // TODO: Make pretty-printer (?) set this flag for Rust strings.
    const UTF_STRING = 0x2;

    const PARSED_SUBRANGE = 0x4; // used during loading
}}
...
pub enum Type {
    ...
    MetaField,
    // TODO: Slice, or maybe use struct with a flag. First check what type Rust slices translate to.
...
impl Type {
    ...
    pub fn kind_name(&self) -> &'static str { match self { Type::Unknown => "unknown", Type::Primitive(_) => "primitive type", Type::Pointer(_) => "pointer", Type::Array(_) => "array", Type::Struct(_) => "struct", Type::Enum(_) => "enum", Type::MetaType => "type", Type::MetaField => "field" } }
}    
...
bitflags! { pub struct TypeFlags: u8 {
    ...
    const POINTERS_NOT_RESOLVED = 0x10;
}}
...
impl Default for TypeInfo { fn default() -> Self { Self {name: "", size: 0, die: DebugInfoOffset(0), flags: TypeFlags::empty(), t: Type::Unknown} } }

impl TypeInfo {
    fn can_have_own_name(&self) -> bool { match &self.t { Type::Pointer(_) | Type::Array(_) => false, _ => true } }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::result::Result<(), fmt::Error> {
        unsafe {
            ...
            match &t.t {
                ...
                Type::Array(a) => write!(f, "[] /{}  @{:x} '{}'", a.stride, (*a.type_).die.0, (*a.type_).name)?,
                Type::Struct(s) => {
                    write!(f, "{{")?;
...
pub struct TypesLoadingShard {
    ...
    // (This is O(n^2) arrays, which seems fine up to maybe hundreds of threads, e.g. 2023 threadripper (not tested).
...
impl Default for NameDedupMessage { fn default() -> Self { Self {name_hash: 0, name: "", type_: ptr::null(), from_alias: None, adjusted_to_avoid_name_collision: false} } }
...
impl TypesLoader {
    ...
    fn dfs(&self, original_shard_idx: usize, vertex_shard_idx: usize, state: &TypeLoadState, mut on_cycle: bool) -> (*const TypeInfo, TypeSpecialInfo) {
        ...
        let info: TypeInfo = match info {
            Err((off, name)) => {
                ...
                if let Some(mut info) = detect_cpp_non_char_8bit_type_from_typedef(name, special) {
                    ...
                    if !name.is_empty() {
                        self.send_dedup_name(original_shard_idx, NameDedupMessage {name, type_: res, from_alias: Some(/*not to be confused with `off`*/ state.offset), ..Default::default()});
                    }
                    return (res, special);
                }
            }
            Ok(mut info) => {
                ...
                match &mut info.t {
                    ...
                    Type::MetaType | Type::MetaField => (), // these don't appear in debug symbols, but we add them as builtins along the way, so end up participating in deduplication unnecessarily
                    Type::Pointer(p) => {
                        unlock();
                ...
                Entry::Occupied(o) => {
                    if flags & FLAG_FINALIZED != 0 {
                        ...
                        if !info.name.is_empty() && dedup.name.is_empty() {
                            self.send_dedup_name(original_shard_idx, NameDedupMessage {name: info.name, type_: res, ..Default::default()});
                        }
        ...
        if !t.name.is_empty() {
            self.send_dedup_name(original_shard_idx, NameDedupMessage {name: t.name, type_: p, ..Default::default()});
    ...
    fn run_dedup_names(&self, shard_idx: usize) {
        ...
        names.sort_unstable_by_key(|k| (k.name, k.type_));
